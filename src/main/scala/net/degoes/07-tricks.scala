...
class NoAllocationBenchmark {
  ...
  def mutable(blackhole: Blackhole): Unit = {
    var i              = 0
    val mutableMetrics = MutableMetricsMap()
    while (i < size) {
...
class NoPatternMatchingBenchmark {
  ...
  def isInstanceOf(blackhole: Blackhole): Unit = {
    ...
    while (i < size) {
      ...
      if (either.isInstanceOf[Left[_, _]]) {
        lefts = lefts + either.asInstanceOf[Left[Int, Int]].value
      } else {
        rights = rights + either.asInstanceOf[Right[Int, Int]].value
      }
      i = i + 1
    }
    blackhole.consume(lefts + rights)
...
class FlattenProductsBenchmark {
  ...
  def flattened(blackhole: Blackhole): Unit = {
    ...
    var total = 0.0
    while (i < size) {
      total = total + (billingsStartDay(i) - billingsEndDay(i)) * billingsDailyRate(i)
      i = i + 1
    }
    blackhole.consume(total)
  }
...
class StackInterpreterBenchmark {
  ...
  val parser2: opt.RouteParser[(String, Int)] = {
  ...
  def interpreted(blackhole: Blackhole): Unit =
    blackhole.consume(parser2.parse("/users/jdegoes/posts/123"))
  ...
  object opt {
    ...
    object RouteParser           {
      ...
      private case class Compiled(instructions: Chunk[Instr], maxStack: Int) {
        // private val threadLocalStack: ThreadLocal[Array[Any]] = new ThreadLocal[Array[Any]] {
        ...
        def execute(path: String): Any = {
          val stack     = Array.ofDim[Any](maxStack)
          var stackSize = 0
          ...
          while (instrIndex < instrLen) {
            val instr0 = instructions(instrIndex)

            (instr0.tag: @switch) match {
              case Tags.Literal    =>
                val instr       = instr0.asInstanceOf[Instr.Literal]
                val valueLength = instr.value.length
                if (path.regionMatches(pathIndex, instr.value, 0, valueLength)) {
                  stack(stackSize) = ()
                  stackSize += 1
                  pathIndex += valueLength
                } else throw DoesNotMatch
              case Tags.LiteralPop =>
                val instr       = instr0.asInstanceOf[Instr.LiteralPop]
                val valueLength = instr.value.length
                if (path.regionMatches(pathIndex, instr.value, 0, valueLength)) {
                  pathIndex += valueLength
                } else throw DoesNotMatch
              ...
              case Tags.IntVar     =>
                ...
                stackSize += 1
              case Tags.Map1       =>
                val instr = instr0.asInstanceOf[Instr.Map1]
                val first = stack(stackSize - 1)
                stack(stackSize - 1) = instr.change(first)
              case Tags.Map2       =>
                val instr  = instr0.asInstanceOf[Instr.Map2]
                val first  = stack(stackSize - 2)
                val second = stack(stackSize - 1)
                stackSize -= 1
                stack(stackSize - 1) = instr.combine(first, second)
              case Tags.Pop        =>
                stackSize -= 1
            }
            instrIndex += 1
        ...
        def fromRouteParser(parser: RouteParser[_]): Compiled = {
          ...
          def loop(parser: RouteParser[_]): Chunk[Instr]    =
            parser match {
              ...
              case CombineRight(left, right) => (loop(left) :+ Instr.Pop) ++ loop(right)
            }

          val instructions  = Chunk.fromIterable(fullyOptimize(loop(parser).toList))
          val (_, maxStack) = instructions.foldLeft((0, 0)) {
            case ((curSize, maxStackSize), instr) =>
      ...
      private sealed abstract class Instr(val tag: Int)
