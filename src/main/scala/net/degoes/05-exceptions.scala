...
class ThrowExceptionBenchmark {
  ...
  def constructException(blackhole: Blackhole): Unit =
    blackhole.consume(MyException("oops!"))
}
...
class TryCatchOverheadBenchmark {
  sealed trait Bool extends Throwable
  object Bool {
    case object False extends Exception("false") with Bool
    case object True  extends Exception("true") with Bool
  }
  ...
  def tryCatch(blackhole: Blackhole): Unit =
    try throw bool
      ...
      case t: Throwable =>
        blackhole.consume(t == Bool.True)
    }

  @Benchmark
  def values(blackhole: Blackhole): Unit = {
    val x = bool
    blackhole.consume(x == Bool.True)
  }
}
